...
\begin{defi}
    % Let \((G,\cdot)\) be a group.
    % A subset \(H\) of  \(G\) is a \textbf{subgroup} of \(G\) if \((H,\cdot)\) is a group. 
    % A  \textbf{subgroup} of a group \((G,\cdot)\) is a subset of \(G\) such that 
    ...
    % \begin{enumerate}[label=(\roman*)]
    %     \item \(H\neq \varnothing\)
    %     \item \(ab\in H\) and \(a^{-1}\in H\) for all \(a,b\in H\)
    % \end{enumerate}
